...
assert.equal(lib.GALLERY_FILTER_FIELDS.length, 25);
assert.equal(new Set(lib.GALLERY_FILTER_FIELDS.map((f) => f.key)).size, 25);
...
assert.equal(
  ...
  "q=cat+videos&kind=video&status=failed&sort=duration_desc&min_duration=5&max_duration=90&min_size=1&max_size=250&min_width=480&max_width=1920&min_height=600&max_height=2400&codec=h264%2C+hevc&min_attempts=1&max_attempts=9&recovery=1&from=2024-01-01&to=2024-12-31&orientation=portrait&assets=with&audio=without&offloaded=with&index=indexed&include=%40creator%2C+%23games&exclude=%23fyp",
);
