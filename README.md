...
- **Finds what you remember.** Local Lens searches timestamped speech and on-screen text imported from analysis tools running on your machine, then jumps straight to the matching moment.
...
Local Lens searches inside a favorite instead of only searching its caption. Import timestamped transcript or OCR segments generated by a tool on your own machine, search both evidence types together or separately, and open a result at the exact matching second in Feed. The Archive does not bundle an AI model and does not send media or analysis to a service.

Imported transcript segments can also appear as timed captions over Feed
videos. Use the **CC** control to show or hide them; captions start off in a
fresh browser and the choice is remembered on that browser. Only transcript
segments are displayed. OCR stays searchable in Local Lens and is never placed
over the video.

The interchange document is intentionally small and tool-neutral:
...
`source` must be `transcript` or `ocr`; times are finite non-negative seconds. A document can contain up to 5,000 favorites and 100,000 total segments within the 64 MB upload limit. The complete document is validated before any indexed text changes, and importing an item replaces only that item's prior analysis.
...
status as unverified legacy placements. The same additive migration creates
empty Local Lens, Memory Lane, Time Machine, and Story Builder tables; it does
not analyze media or rewrite existing rows. Feed captions use only transcript
segments you explicitly import into Local Lens, so upgrading does not require a
caption or OCR backfill.
